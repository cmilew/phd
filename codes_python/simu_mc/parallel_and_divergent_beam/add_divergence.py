...
input_phsp_path = os.path.join(
    os.path.dirname(__file__), "data/parallel_beam_phsp_1.000E+6.root"
)
phsp_name = "parallel_beam_phsp_1.000E+6"
output_phsp_name = os.path.join(
    os.path.dirname(__file__), "data/divergent_beam_phsp_1.000E+6.root"
)
...
# save script execution time in .txt
with open(
    os.path.join(os.path.dirname(__file__), "data/execution_time.txt"), "w"
) as file:
    file.write(f"Temps d'exécution : {execution_time:.4f} secondes\n")
